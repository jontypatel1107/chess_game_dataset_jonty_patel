...
router.get("/top-openings", asyncHandler(async (req, res) => successResponse(res, 200, "Top openings fetched successfully", require("../data/openings").slice(0, 10))));
...
router.get("/rated-games", asyncHandler(async (req, res) => successResponse(res, 200, "Rated games fetched successfully", { total: await Game.countDocuments({ "ratingChange.white": { $ne: 0 } }) })));
router.get("/unrated-games", asyncHandler(async (req, res) => successResponse(res, 200, "Unrated games fetched successfully", { total: await Game.countDocuments({ "ratingChange.white": 0, "ratingChange.black": 0 }) })));
